...
interface ApiResponse {
  ...
  data: {
    ...
    deliveryType: "terminal" | "doorstep";
    status: "PENDING" | "SHIPPED" | "DELIVERED";
  };
}
...
const transformApiData = (
  ...
  const getTrackingProgress = (status: string) => {
    const statusLower = status.toLowerCase();
    const isTerminal = data.deliveryType === "terminal";
    ...
    const baseProgress = [
      ...
      },
      {
        ...
        description:
          statusLower === "shipped"
            ? `Package has left our facility and is ${
                isTerminal
                  ? "on its way to the pickup terminal"
                  : "on its way to you"
              }`
            : statusLower === "delivered"
            ? `Package left our facility and was ${
                isTerminal
                  ? "on its way to the pickup terminal"
                  : "on its way to you"
              }`
            : `Package will leave our facility and be ${
                isTerminal
                  ? "on its way to the pickup terminal"
                  : "on its way to you"
              }`,
  ...
  // Determine destination address based on delivery type
  const getDestinationAddress = () => {
    if (data.deliveryType === "terminal" && data.buyer.terminalAddress) {
      return capitalizeWords(data.buyer.terminalAddress);
    }
...
export default function TrackOrderPage() {
  ...
  // Helper function to determine if delivery is terminal pickup
  const isTerminalPickup = () => {
    return apiData?.data?.deliveryType === "terminal";
  };

  // Helper function to get delivery address label
  const getDeliveryAddressLabel = () => {
    return isTerminalPickup() ? "Terminal Pickup" : "Delivery Address";
  };

  // Helper function to get delivery address icon
  const getDeliveryAddressIcon = () => {
    return isTerminalPickup() ? (
        ...
        <div className="space-y-4 lg:grid lg:grid-cols-3 lg:gap-6 lg:space-y-0">
          ...
          <div className="space-y-4 lg:col-span-2">
            ...
            <Card>
              <CardHeader className="pb-3 sm:pb-6">
                ...
                <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                  Follow your order's journey from start to{" "}
                  {isTerminalPickup() ? "pickup terminal" : "delivery"}
                </p>
              </CardHeader>
              <CardContent className="p-3 md:p-6">
                <div className="space-y-6">
                    ...
                    <div className="flex items-center justify-between mb-2">
                      {orderData.trackingProgress.map((step, index) => (
                        ...
                        >
                          ...
                          <span className="text-xs font-medium capitalize">
                            {step.status === "delivered" && isTerminalPickup()
                              ? "ready"
                              : step.status}
                          </span>
                        </div>
          ...
          <div className="space-y-4">
              ...
              <CardContent className="space-y-3 md:space-y-4 p-3 md:p-6">
                <div className="flex items-center gap-2">
                  ...
                  <Badge
                    ...
                    className="text-xs sm:text-sm capitalize"
                  >
                    {orderData.status === "delivered" && isTerminalPickup()
                      ? "ready for pickup"
                      : orderData.status}
                  </Badge>
                </div>
                    ...
                    <span>
                      {isTerminalPickup()
                        ? orderData.status === "delivered"
                          ? "Ready for pickup since:"
                          : "Estimated arrival at terminal:"
                        : "Estimated Delivery:"}
                    </span>
                  </div>
                  <p className="font-medium text-sm md:text-base">
            ...
            <Card>
              <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-base md:text-lg">
                  {getDeliveryAddressIcon()}
                  {isTerminalPickup() ? "Pickup Details" : "Delivery Details"}
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3 sm:space-y-4 p-3 sm:p-6">
                <div className="grid grid-cols-1 md:grid-cols-1 gap-3">
