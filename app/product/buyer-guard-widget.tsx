...
export function BuyerGuardWidget({ productData }: BuyerGuardWidgetProps) {
  ...
  const getFeeMessage = () => {
    ...
    switch (productData.returnShippingFee) {
      ...
      case "SHARED":
        return `the product seller covers ${
          productData.supplierShare ?? "a portion"
        }% of the return shipping fee.`;
      default:
        return "";
    }
  };
  ...
  return (
    <>
      ...
      {isOpen && (
        <dialog
          open
          className="fixed bottom-0 right-0 md:bottom-auto md:right-4 md:top-1/2 md:transform md:-translate-y-1/2 
                     w-full md:w-96 md:max-w-sm max-h-[90vh] md:max-h-[600px] bg-background 
                     rounded-t-2xl md:rounded-2xl shadow-2xl z-50 overflow-hidden 
        ...
        >
          ...
          {/* Scrollable Content */}
          <div className="flex flex-col max-h-[calc(90vh-4rem)] md:max-h-[550px] overflow-hidden">
            <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5 pb-10">
              ...
              {/* Terms — moved up with clearer spacing */}
...
function CollapsibleTerms({ terms }: { terms: string }) {
  ...
  const truncatedTerms =
    terms.length > 1500 ? terms.substring(0, 1500) + "..." : terms;
    ...
    <div>
      ...
      {isExpanded && (
        <div className="mt-2 p-3 bg-muted/20 rounded-lg border border-border">
          <div className="max-h-72 overflow-y-auto text-xs text-muted-foreground leading-relaxed break-words whitespace-pre-wrap">
            {truncatedTerms}
          </div>
        </div>
      )}
