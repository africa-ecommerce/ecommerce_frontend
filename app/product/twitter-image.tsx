...
// // Image metadata
// export const alt = "Product on Twitter";
// export const size = {
//   width: 1200,
//   height: 630,
// };
...
//     // Get the product ID from search params
...
//                   bottom: 0,
//                   left: 0,
//                   backgroundColor: "rgba(79, 70, 229, 0.9)", // Indigo color from your original example
//                   color: "white",
//                   padding: "8px 16px",
...
// Configure as edge function for best performance
export const runtime = 'edge';

/**
 * Twitter Image Route that redirects to the API endpoint
 * This approach eliminates the need to handle searchParams directly in twitter-image.tsx
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const ref = searchParams.get('ref') || 'null'; // Use 'null' as a fallback for the path parameter
    const platform = searchParams.get('platform');

    // Exit early if no product ID is provided
    if (!productId) {
      return new Response('Product ID is required', { status: 400 });
    }
    
    // Construct the URL to the actual OG image API endpoint
    const baseUrl = process.env.NEXT_PUBLIC_BASE_URL || '';
    
    // Build the redirect URL to your existing API that already works
    let redirectUrl = `${baseUrl}/api/og/${productId}/${ref}.png`;
    
    // Add platform parameter if it exists
    if (platform) {
      redirectUrl += `?platform=${platform}`;
    }
    
    // Redirect to the API endpoint
    return NextResponse.redirect(redirectUrl);
  } catch (error) {
    console.error('Error in Twitter image redirect:', error);
