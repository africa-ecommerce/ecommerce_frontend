...
export const SingleProduct = ({ productId, referralId, platform }: SingleProductCartProps) => {
  ...
  const { data, error, isLoading, mutate } = useSWR(
    currentProductId
      ? `/api/public/products/${currentProductId}${currentReferralId ? `/${currentReferralId}` : ""}`
      : null,
    async (url) => {
      const response = await fetch(url, {
        method: "GET",
  ...
  useEffect(() => {
    if (orderSummaries.length > 0 && productData && currentProductId && !hasRestoredState) {
      const existingOrders = orderSummaries.filter((order) => order.item.id === currentProductId)
            ...
            if (order.item.variationId) {
              ...
              if (variation) {
                restoredVariations.push({
                  variation,
                  quantity: order.item.quantity,
                  selectedColor: order.item.selectedColor || variation.colors[0],
                })
              }
            }
          })
  ...
  }, [orderSummaries, currentProductId, productData, hasVariations, hasRestoredState])
  ...
  const handleVariationToggle = (variation: ProductVariation) => {
    ...
    if (isSelected) {
      setSelectedVariations((prev) => prev.filter((sv) => sv.variation.id !== variation.id))
    } else {
  ...
  const pricing = useMemo(() => {
    ...
    if (hasVariations && selectedVariations.length > 0) {
      ...
      for (const sv of selectedVariations) {
        ...
        subtotal += itemTotal
        items.push({
          name: getVariationDisplayName(sv.variation, sv.selectedColor),
          price,
          quantity: sv.quantity,
          total: itemTotal,
        })
  ...
  const handleCheckout = () => {
    ...
    // Remove existing orders for this product first
    const filteredOrders = orderSummaries.filter((order) => order.item.id !== productData.id)
    ...
    if (hasVariations && selectedVariations.length > 0) {
      ...
      for (const sv of selectedVariations) {
        const productItem = {
          ...
          image: productData.images?.[0] || "/placeholder.svg",
          variationId: sv.variation.id,
          variationName: getVariationDisplayName(sv.variation, sv.selectedColor),
          supplierId: productData.supplierId,
        }
              ...
              <CardContent className="p-6">
                <div className="flex flex-col md:flex-row gap-6">
                  ...
                  <div className="flex-1">
                    ...
                    {!hasVariations && productData?.colors && productData.colors.length > 0 && (
                      <div className="mb-6">
                        ...
                        <div>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[320px] md:max-h-[400px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-muted scrollbar-track-transparent">
                            {productData.variations.map((variation) => {
                              ...
                              const currentSelectedColor = selectedVariation?.selectedColor || ""
                              ...
                              return (
                                ...
                                >
                                  <div className="p-4 cursor-pointer" onClick={() => handleVariationToggle(variation)}>
                                    ...
                                    {isSelected && variation.colors && variation.colors.length > 0 && (
                                      <div className="mb-3 pt-3 border-t border-muted">
                                        <Label className="text-xs font-medium mb-2 block">Choose Color:</Label>
                                        <Popover>
                                          <PopoverTrigger asChild>
                                            <Button
                                              variant="outline"
                                            ...
                                            >
                                              {currentSelectedColor ? (
                                                <>
                                                  <ColorSwatch color={currentSelectedColor} size="w-3 h-3" />
                                                  <span className="capitalize text-xs">{currentSelectedColor}</span>
                                                </>
                                              ) : (
                                                "Select color"
                                              )}
                                          ...
                                          <PopoverContent className="w-full p-2" onClick={(e) => e.stopPropagation()}>
                                            <div className="grid grid-cols-2 gap-1">
                                              {variation.colors.map((color) => (
                                                <Button
                                                  key={color}
                                                  variant={currentSelectedColor === color ? "default" : "ghost"}
                                                  size="sm"
                                                  className="justify-start gap-2 h-auto p-2"
                                                  onClick={(e) => {
                                                    e.stopPropagation()
                                                    updateVariationColor(variation.id, color)
                                                  }}
                                                >
                                                  <ColorSwatch color={color} size="w-3 h-3" />
