...
export default function PWAInstallPrompt() {
  ...
  const handleGotIt = async () => {
    // Try to trigger the Web Share API to open the share menu
    if (navigator.share) {
      try {
        await navigator.share({
          title: 'Pluggn',
          text: 'Add Pluggn to your home screen for the best experience!',
          url: window.location.href
        })
        // Close modal after sharing (or attempting to share)
        setShowIOSModal(false)
        localStorage.setItem("pwa-dismissed", Date.now().toString())
      } catch (error) {
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 animate-scale-in">
            ...
            <div className="flex gap-3">
              ...
              <Button onClick={handleGotIt} className="flex-1 bg-[#FF7A21] hover:bg-[#ff8c42]">
                Got It
              </Button>
            </div>
          </div>
