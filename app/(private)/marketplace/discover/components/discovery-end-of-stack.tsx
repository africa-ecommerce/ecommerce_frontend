...
export function DiscoveryEndOfStack() {
  return (
    <div className="relative w-full min-h-[100vh] flex items-center justify-center px-[5vw] py-[5vh] overflow-hidden">
      {/* Floating sparkles */}
      <FloatingSparkle delay={0} x={-15} y={-25} />
      <FloatingSparkle delay={0.5} x={18} y={-18} />
      <FloatingSparkle delay={1} x={-12} y={15} />
      <FloatingSparkle delay={1.5} x={14} y={22} />
      <FloatingSparkle delay={0.8} x={-20} y={5} />
      <FloatingSparkle delay={1.2} x={16} y={-8} />

      {/* Background stacked cards */}
      <div className="absolute inset-0 flex items-center justify-center">
        <motion.div
          initial={{ scale: 0.85, y: "2vmin", rotate: -8, opacity: 0 }}
          animate={{ scale: 0.95, y: "1vmin", rotate: -7, opacity: 0.4 }}
          transition={{ type: "spring", stiffness: 200, damping: 25, delay: 0.1 }}
          className="absolute w-[80vw] max-w-[400px] aspect-[4/5] bg-white/40 rounded-[3vmin] shadow-xl backdrop-blur-sm"
          style={{ zIndex: 1 }}
        />
        <motion.div
          initial={{ scale: 0.9, y: "1vmin", rotate: 6, opacity: 0 }}
          animate={{ scale: 0.97, y: "0.8vmin", rotate: 7, opacity: 0.6 }}
          transition={{ type: "spring", stiffness: 200, damping: 25, delay: 0.2 }}
          className="absolute w-[83vw] max-w-[420px] aspect-[4/5] bg-white/50 rounded-[3vmin] shadow-xl backdrop-blur-sm"
          style={{ zIndex: 2 }}
        />
      </div>
      ...
      {/* Main content card */}
      <motion.div
        initial={{ scale: 0.9, y: "5vmin", opacity: 0 }}
        animate={{ scale: 1, y: 0, opacity: 1 }}
        transition={{ type: "spring", stiffness: 250, damping: 20, delay: 0.3 }}
        className="relative w-[88vw] max-w-[440px] aspect-[4/5] bg-white rounded-[3vmin] shadow-2xl overflow-hidden"
        style={{ zIndex: 10 }}
      >
        {/* Gradient overlay */}
        <div className="absolute top-0 left-0 right-0 h-[15vmin] bg-gradient-to-b from-orange-50/80 to-transparent" />

        {/* Content */}
        <div className="relative h-full flex flex-col items-center justify-center px-[6vw] py-[6vh] text-center">
          {/* Animated icon */}
          <motion.div
            initial={{ scale: 0, rotate: -180 }}
            animate={{ scale: 1, rotate: 0 }}
          ...
          >
            <div className="relative">
              <motion.div
                ...
                className="w-[18vmin] h-[18vmin] bg-gradient-to-br from-orange-400 to-orange-500 rounded-full flex items-center justify-center shadow-lg"
              >
                <Sparkles className="w-[8vmin] h-[8vmin] text-white" strokeWidth={2} />
              </motion.div>
              {/* Pulsing ring */}
              <motion.div
                animate={{
          ...
          <motion.h2
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.6 }}
            className="font-bold text-gray-900 mb-[2vmin] text-balance text-[clamp(1.5rem,4vw,2.2rem)]"
          >
            Your stack is finished{" "}
          ...
          <motion.p
            ...
            animate={{ opacity: 1, y: 0 }}
            transition={{ delay: 0.7 }}
            className="text-gray-600 leading-relaxed mb-[5vmin] max-w-[45ch] text-[clamp(0.9rem,2.5vw,1.1rem)]"
          >
            Come back after 12 hours to receive your new stack. In the meantime,
            visit your product dashboard and share a stunning image of your
            product — you’ll love it.
          </motion.p>
          ...
          >
            <Link
              href="/dashboard/product"
              className="group relative inline-flex items-center justify-center px-[8vmin] py-[2.5vmin] text-[clamp(0.9rem,2.5vw,1.1rem)] font-semibold text-white bg-gradient-to-r from-orange-500 to-orange-600 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 active:scale-95 overflow-hidden"
            >
              {/* Shimmer effect */}
              <motion.div
                className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent"
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            transition={{ delay: 1 }}
            className="absolute bottom-[4vmin] left-0 right-0 flex justify-center gap-[1vmin]"
          >
            {[...Array(3)].map((_, i) => (
              <motion.div
                key={i}
                ...
                className="w-[1vmin] h-[1vmin] bg-orange-400 rounded-full"
              />
            ))}
          </motion.div>
        </div>
...
function FloatingSparkle({ delay, x, y }: { delay: number; x: number; y: number }) {
  return (
    ...
    >
      <Sparkles className="w-[3vmin] h-[3vmin] text-orange-400/60" fill="currentColor" />
    </motion.div>
  );
}
