...
export function ProductSpecifications({
  ...
  const formatColors = (colors: string[]) =>
    colors.map((color) => capitalizeWords(color)).join(", ");
  ...
  const formatValue = (key: string, value: any) => {
    ...
    switch (key) {
      case "colors":
        if (Array.isArray(value) && value.length > 0) return formatColors(value);
        return null;
      case "stock":
      case "moq": // ✅ added to handle MOQ as number
        return value;
      default:
        return typeof value === "string"
          ? value.charAt(0).toUpperCase() + value.slice(1)
  ...
  // ✅ Include moq in allowed specs
  const allowedSpecs = ["size", "colors", "stock", "moq"];
