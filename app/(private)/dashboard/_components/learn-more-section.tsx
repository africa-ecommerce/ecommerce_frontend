...
export default function LearnMoreSection({ onBack, modal, onClose }: LearnMoreSectionProps) {
  const [completedSteps, setCompletedSteps] = useState<Record<string, boolean>>(
    {}
  );
  const [expandedSections, setExpandedSections] = useState<
  ...
  const contentComponent = (
    <div className={`animate-fade-in ${modal ? 'w-full' : 'md:max-w-[400px] max-w-[340px]'}`}>
      {/* Header */}
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center">
          {onBack && !modal && (
  ...
  // If modal is true, wrap in modal overlay
  if (modal) {
    return (
  ...
  // If not modal, return content as is
