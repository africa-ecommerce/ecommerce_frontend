...
import { ArrowLeft, Package, RotateCcw, AlertTriangle, Loader2 } from "lucide-react"
...
const OrderCard = ({ item, type }: { item: OrderItem; type: "issues" | "returns" }) => {
        ...
        <div className="space-y-3">
          ...
          <div className="pt-2 border-t">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">{type === "issues" ? "Paused Items" : "Returned Items"}</span>
  ...
  if (error) {
    return (
      <Alert variant="destructive" className="animate-fade-in">
        ...
        <AlertDescription>Failed to load {type}. Please try again later.</AlertDescription>
      </Alert>
    )
  }
    ...
    return (
      <Card className="animate-fade-in">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="text-base font-semibold mb-2">No {type === "issues" ? "Issues" : "Returns"} Found</h3>
          <p className="text-muted-foreground text-center">
            You don't have any {type === "issues" ? "paused orders" : "returned items"} at the moment.
          </p>
        </CardContent>
      </Card>
    )
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 grid-cols-1">
        {data.data.map((item, index) => (
          <div key={`${item.orderNumber}-${index}`} style={{ animationDelay: `${index * 100}ms` }}>
...
export function IssueSection({ onBack, userType }: IssueSectionProps) {
  ...
  const issuesEndpoint = userType === "PLUG" ? "/api/orders/plug/paused" : "/api/orders/supplier/paused"

  const returnsEndpoint = userType === "PLUG" ? "/api/orders/plug/returned" : "/api/orders/supplier/returned"
