...
//       console.log("debouncedData", data);
...
// FIX: Modified fetcher to work correctly with SWR
const fetcher = async (url) => {
  // Extract the subdomain from the URL query string if present
...
export default function PublishDialog({
  ...
  // IMPROVED: Validate subdomain format client-side before checking with API
  const validateSubdomainFormat = (name: string) => {
    if (!name || name.trim() === "") {
      setInputError("Site name is required");
      return false;
  ...
  // IMPROVED: Only make API call if the subdomain format is valid
  const isValidFormat = debouncedSubdomain
    ? validateSubdomainFormat(debouncedSubdomain)
    : false;

  // FIX: Properly set up SWR key with query parameters instead of array
  const { data, error, isValidating } = useSWR(
    debouncedSubdomain && debouncedSubdomain.trim() !== "" && isValidFormat
      ? `/api/site/check-subdomain?subdomain=${encodeURIComponent(debouncedSubdomain)}`
      : null,
    fetcher
  );
  ...
  useEffect(() => {
    if (debouncedSubdomain && debouncedSubdomain.trim() !== "") {
      // If the format is invalid, don't proceed with API checking
      if (!isValidFormat) {
        setNameAvailable(false);
        return;
      }
  ...
  }, [data, error, isValidating, debouncedSubdomain, isValidFormat]);

  // IMPROVED: Handle subdomain input changes
                      ...
                      <div className="flex">
                        ...
                        <div className="relative flex-1">
                          <input
                            ...
                            pattern="[a-zA-Z0-9-]+" // HTML5 pattern validation for allowed characters
                          />
                          {isValidating && (
                            <div className="absolute right-2 top-1/2 transform -translate-y-1/2">
                              <Loader2 className="h-4 w-4 animate-spin text-slate-400" />
