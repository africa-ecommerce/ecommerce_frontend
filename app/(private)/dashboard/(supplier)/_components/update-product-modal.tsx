...
import { useState, useRef, useEffect } from "react"
import { motion, AnimatePresence } from "framer-motion"
import { Check, ChevronLeft, ChevronRight, HelpCircle, ImageIcon, Loader2, Plus, Trash2, Upload, X } from "lucide-react"
...
export function UpdateProductModal({ open, onOpenChange, productId, itemData }: EditProductModalProps) {
  const [currentStep, setCurrentStep] = useState(0)
  const [direction, setDirection] = useState(0)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const dropAreaRef = useRef<HTMLDivElement>(null)
    ...
    if (data) {
      ...
      if (data.variations && data.variations.length > 0) {
        ...
        const updatedVariations = data.variations.map((variation) => ({
          ...variation,
          colors: Array.isArray(variation.colors) ? variation.colors : variation.color ? [variation.color] : [],
        }))
        setValue("variations", updatedVariations)
      } else {
      ...
      <div className="fixed inset-0 z-[100] flex items-end justify-center bg-black/50 backdrop-blur-sm md:items-center">
        ...
        >
          <div className="flex items-center justify-between border-b px-4 py-3 md:px-6">
            ...
            <Button variant="ghost" size="icon" onClick={() => closeModal()} className="h-9 w-9 rounded-full">
              <X className="h-5 w-5" />
              <span className="sr-only">Close</span>
            </Button>
          </div>
                ...
                {currentStep === 4 && (
                  <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                              ...
                              <div key={variation.id} className="rounded-lg border p-3">
                                <div className="grid grid-cols-2 gap-4">
                                  <div>
                                    ...
                                    <p className="font-medium capitalize">
                                      {variation.size || (variation.colors && variation.colors.length > 0)
                                        ? [variation.size, variation.colors?.join(", ")].filter(Boolean).join(" / ")
                                        : `Variation ${index + 1}`}
                                    </p>
                                  </div>
                                  <div>
                                    ...
                                    {variation.colors && variation.colors.length > 0 ? (
                                      <div className="flex flex-wrap gap-1 mt-1">
                                        {variation.colors.map((colorValue) => {
                                          const color = PREDEFINED_COLORS.find((c) => c.value === colorValue)
                                          return (
                                            <Badge
                                              key={colorValue}
                      ...
                      {imagePreviews.length > 0 && (
                        <div className="rounded-xl border bg-muted/30 p-5">
                          <h4 className="mb-3 text-sm font-medium">Images ({imagePreviews.length})</h4>
                          <div className="grid grid-cols-3 gap-3 md:grid-cols-4">
                            {imagePreviews.map((url, index) => (
