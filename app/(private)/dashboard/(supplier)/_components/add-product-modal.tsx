...
import { ChevronLeft, ChevronRight, X, Check, Plus, Upload, ImageIcon, Trash2, Loader2 } from "lucide-react"
import { cn } from "@/lib/utils"
import { useFormResolver } from "@/hooks/useFormResolver";
import { productFormSchema, type ProductFormData, type Variation} from "@/zod/schema"
import { ColorPicker } from "./color-picker" // Import ColorPicker component
import { mutate } from "swr"
import { categoryRecommendations, PRODUCT_CATEGORIES } from "@/app/constant"
import { errorToast, successToast } from "@/components/ui/use-toast-advanced"
...
export function AddProductModal({
  ...
  const {
    ...
    },
    {
      ...
      price: undefined,
      stock: undefined,
      color: [], // Initialize as empty array
    },
  )
  ...
  const updateVariation = (
    index: number,
    field: string,
    value: string | number | string[], // Add string[] for color arrays
  ) => {
    const updatedVariations = [...(formData.variations || [])] as Variation[]
  ...
  const getColorDisplay = (colorValues: string[]) => {
    return colorValues.map((value) => PREDEFINED_COLORS.find((c) => c.value === value)).filter(Boolean)
  }
                      ...
                      <div className="flex items-center justify-between rounded-lg bg-muted/30 p-4">
                        ...
                        {formData.hasVariations && (
                          <Button
                            ...
                            size="sm"
                            onClick={addVariation}
                            className="h-9 shadow-sm bg-transparent"
                            type="button"
                          >
                            <Plus className="mr-1 h-4 w-4" /> Add
                          </Button>
                            ...
                            <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
                              ...
                              <div className="space-y-3 pb-4">
                                <ColorPicker
                                  ...
                                  id={`color-${index}`}
                                  value={variation.color || []}
                                  onChange={(colors) => updateVariation(index, "color", colors)}
                                  placeholder="Select colors..."
                                />
                              </div>

                              <div className="space-y-3 pb-4 md:col-span-2">
                    ...
                    <div className="grid gap-6 grid-cols-1 md:grid-cols-2">
                      ...
                      <div className="space-y-3">
                        <ColorPicker
                          ...
                          onChange={(colors) => setValue("color", colors)}
                          placeholder="Select colors..."
                        />
                      </div>
                    </div>
                  ...
                  <div className="space-y-6">
                    <div className="rounded-xl border bg-muted/30 p-6">
                      ...
                      <div className="grid gap-4 text-sm">
                        ...
                        {!formData.hasVariations && formData.color && formData.color.length > 0 && (
                          <div className="flex justify-between items-center">
                            <span className="text-muted-foreground">Colors:</span>
                            <div className="flex flex-wrap gap-1">
                              {getColorDisplay(formData.color).map((color) => (
                                <Badge key={color?.value} variant="secondary" className="flex items-center gap-1">
                                  <div
                          ...
                          <div>
                            ...
                            <div className="mt-2 space-y-2">
                              {formData.variations?.map((variation, index) => (
                                <div key={variation.id} className="rounded-lg bg-background p-3">
                                  <div className="flex justify-between items-start">
                                    <div>
                                      <p className="font-medium">Size: {variation.size}</p>
                                      {variation.color && variation.color.length > 0 && (
                                        <div className="flex items-center gap-2 mt-1">
                                          <span className="text-xs text-muted-foreground">Colors:</span>
                                          <div className="flex flex-wrap gap-1">
                                            {getColorDisplay(variation.color).map((color) => (
                                              <Badge
                                                key={color?.value}
                                                variant="outline"
                                    ...
                                    <span className="text-sm font-medium">Stock: {variation.stock}</span>
                                  </div>
                                </div>
                              ))}
                            </div>
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{formData.imageUrls?.length || 0} uploaded</span>
                        </div>
                      </div>
                    </div>
                  </div>
