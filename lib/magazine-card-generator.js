...
export async function createLuxuryMagazineCard({
  ...
  customization = {
    ...
    imageHeight: 0.53,   // Reduce to make image smaller, increase to make larger
    
    // Text and element sizes (multipliers)
    titleFontSize: 1.3,    // Reduce to make title smaller
    priceFontSize: 1.5,    // Reduce to make price smaller
    taglineFontSize: 0.8,  // Reduce to make tagline smaller
...
async function renderSellerSection(ctx, {
  ...
  const containerHeight = cardHeight * 0.16; // Increase from 0.12 to 0.16 or higher  
