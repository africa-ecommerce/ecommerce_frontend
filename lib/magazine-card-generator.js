...
export async function createEnhancedMagazineCard({
  ...
  try {
    // Scene setup - KEEPING ORIGINAL CAMERA SETTINGS
    scene = resourceManager.track(new THREE.Scene());
    camera = new THREE.PerspectiveCamera(35, finalWidth / finalHeight, 0.1, 1000); // Original FOV and settings
    camera.position.set(0, 0, 5); // Original camera position
    ...
    const ambientLight = new THREE.AmbientLight(0xffffff, 0.9); // Original intensity
    const directionalLight = new THREE.DirectionalLight(0xffffff, 0.6); // Original intensity
    directionalLight.position.set(5, 5, 10); // Original position
    scene.add(ambientLight, directionalLight);
    ...
    geometry = resourceManager.track(new THREE.BoxGeometry(2.5, 3.5, 0.1)); // Original dimensions
    
    // Use MeshBasicMaterial like original for consistent look
    const sideMaterial = resourceManager.track(new THREE.MeshBasicMaterial({ color: 0xffffff }));
    const frontMaterial = resourceManager.track(new THREE.MeshBasicMaterial({ map: texture }));
    const backMaterial = resourceManager.track(new THREE.MeshBasicMaterial({ color: 0xf0f0f0 }));
    ...
    // Add text overlay using ORIGINAL LAYOUT AND POSITIONING
    await addOriginalLayoutTextOverlay(ctx, {
      finalWidth,
      finalHeight,
      productName,
      productPrice,
...
// ORIGINAL LAYOUT TEXT OVERLAY - keeping exact same positioning and fonts
async function addOriginalLayoutTextOverlay(ctx, { 
  finalWidth, 
  finalHeight, 
  productName, 
  productPrice, 
  ...
  // Calculate scale factor to maintain original proportions
  const scaleX = finalWidth / 800; // Assuming original was designed for ~800px width
  const scaleY = finalHeight / 600; // Assuming original was designed for ~600px height
  ...
  // INSPIRE - ORIGINAL STYLING
  ctx.fillStyle = "#523a1e";
  ctx.font = `bold ${112 * scale}px Georgia, serif`;
  ctx.fillText("INSPIRE", 60 * scale, 140 * scale);

  // By PLUGGN - ORIGINAL STYLING
  ctx.font = `italic bold ${32 * scale}px serif`;
  ctx.fillStyle = "#222";
  ctx.fillText("By PLUGGN", finalWidth - (280 * scale), 140 * scale);

  // NEW Badge - ORIGINAL STYLING
  ctx.fillStyle = "#000";
  ctx.fillRect(60 * scale, 160 * scale, 90 * scale, 50 * scale);
  ctx.fillStyle = "#fff";
  ctx.font = `bold ${22 * scale}px sans-serif`;
  ctx.fillText("NEW", 85 * scale, 193 * scale);

  // ELEVATE YOUR LOOK - ORIGINAL STYLING
  ctx.fillStyle = "#000";
  ctx.font = `bold ${50 * scale}px sans-serif`;
  ctx.fillText("ELEVATE", 60 * scale, 260 * scale);
  ctx.fillText("YOUR LOOK", 60 * scale, 320 * scale);

  // Product Name - ORIGINAL STYLING
  ctx.font = `bold ${32 * scale}px sans-serif`;
  ctx.fillStyle = "#6d4b28";
  ctx.fillText(productName.toUpperCase(), 60 * scale, 370 * scale);

  // Price - ORIGINAL STYLING
  ctx.font = `bold ${30 * scale}px sans-serif`;
  ctx.fillStyle = "#000";
  ctx.fillText(`₦${productPrice.toLocaleString()}`, 70 * scale, 425 * scale);

  // Call to action - ORIGINAL STYLING
  ctx.font = `bold ${28 * scale}px sans-serif`;
  ctx.fillText(`CLICK TO BUY`, 70 * scale, finalHeight - (68 * scale));

  // Circular inset image - ORIGINAL POSITIONING
  try {
    const timeoutDuration = deviceTier === 'mobile-low' ? 3000 : 5000;
    const insetImg = await Promise.race([
      loadSecondaryImageHQ(secondaryImageUrl),
    ...
    if (insetImg) {
      const cx = finalWidth - (220 * scale);
      const cy = finalHeight - (190 * scale);
      ...
      // Creator name - ORIGINAL STYLING
      ctx.font = `bold ${26 * scale}px sans-serif`;
      ctx.fillStyle = "#000";
      ctx.fillText(creatorName, cx - (80 * scale), finalHeight - (50 * scale));
    }
  } catch (error) {
    console.warn("Failed to load secondary image:", error);
  }
