...
export async function createLuxuryMagazineCard({
  ...
  dimensions = { width: 520, height: 760 },
  ...
  customization = {
    ...
    priceFontSize: 1.4,    // Reduce to make price smaller
    taglineFontSize: 0.7,  // Reduce to make tagline smaller
...
async function renderSellerSection(ctx, {
  ...
  const containerHeight = cardHeight * 0.11; // Increase from 0.12 to 0.16 or higher  
  ...
  // Seller name
  ctx.font = `500 ${baseFontSize * 0.85}px "Inter", -apple-system, BlinkMacSystemFont, sans-serif`;
  ctx.fillStyle = '#2c2c2c';
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ...
  ctx.font = `300 ${baseFontSize * 0.75}px "Inter", -apple-system, BlinkMacSystemFont, sans-serif`;
  ctx.fillStyle = '#8b7355';
  ctx.letterSpacing = '0.05em';
