...
export async function createLuxuryMagazineCard({
  ...
  customization = {
    ...
    imageStartY: 0.13,   // Reduce to move image up, increase to move down
    imageHeight: 0.58,   // Reduce to make image smaller, increase to make larger
    ...
    logoSize: 0.11,        // Reduce to make logo smaller
    
    // Background colors (you can modify these)
    cardColors: {
      primary: '#fcf2e6',     // Main card background (more brown)
...
async function renderHeader(ctx, { cardX, cardY, cardWidth, cardHeight, finalWidth, finalHeight, deviceCaps, customization }) {
  ...
  // Badge text
  const baseFontSize = Math.max(12, finalWidth / 50);
  ctx.font = `500 ${baseFontSize * 1.3}px "Inter", -apple-system, BlinkMacSystemFont, sans-serif`;
  ctx.fillStyle = '#8B7355';
  ctx.textAlign = 'center';
  ctx.textBaseline = 'middle';
  ctx.letterSpacing = '0.2em';
