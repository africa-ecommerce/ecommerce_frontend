...
async function renderMagazineCard(ctx, {
  ...
  const cardWidth = finalWidth * 0.85;
  const cardHeight = finalHeight * 0.9;
...
async function renderHeader(ctx, { cardX, cardY, cardWidth, cardHeight, finalWidth, finalHeight, deviceCaps }) {
  ...
  ctx.font = `500 ${baseFontSize * 0.8}px "Inter", -apple-system, BlinkMacSystemFont, sans-serif`;
...
async function renderProductDetails(ctx, {
  ...
  const detailsY = cardY + cardHeight * 0.58;
  const padding = cardWidth * 0.05;
...
async function renderSellerSection(ctx, {
  ...
  const sellerY = cardY + cardHeight * 0.75;
  const padding = cardWidth * 0.05;
  ...
  // Curated by text
  ctx.font = `300 ${baseFontSize * 0.7}px "Inter", -apple-system, BlinkMacSystemFont, sans-serif`;
  ctx.fillStyle = '#8b7355';
  ctx.letterSpacing = '0.05em';
