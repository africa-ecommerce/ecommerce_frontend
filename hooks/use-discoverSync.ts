...
export function useDiscoverSync() {
  ...
  const sync = useCallback(async () => {
    ...
    try {
      ...
      if (changed && hasChanges) {
        const acceptedChunks = chunkArray(accepted, 10);
        const rejectedChunks = chunkArray(rejected, 10);

        console.log(
          `📦 Sending ${acceptedChunks.length} accepted batches and ${rejectedChunks.length} rejected batches`
        );

        // Send batches sequentially to avoid race conditions
        for (
          let i = 0;
          ...
          const batch: SyncPayload = {
            ...
            swipesCount: i === 0 ? swipesCount : 0, // only send swipesCount once
            timestamp: Date.now(),
          };

          try {
            await trigger(batch);
            console.log("✅ Batch synced:", batch);
          } catch (err) {
            console.error("❌ Batch sync failed:", err);
            await queueSync(batch); // queue failed batch
          }
        }

        // ✅ Update snapshot and reset local state
        lastSyncedData.current = {
          accepted: [...accepted],
          rejected: [...rejected],
          swipesCount,
