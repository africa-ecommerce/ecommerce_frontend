...
export function useProductFetcher() {
  ...
  // Set mounted state - use useEffect instead of useLayoutEffect to avoid hydration issues
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  // Fetch all products manually (not using SWR to avoid conditional hooks)
  const fetchProducts = useCallback(async () => {
    if (!mounted || hasInitialized.current || isProcessing.current) return;
  ...
  // Initial fetch when component mounts or dependencies change
  useEffect(() => {
    if (mounted && !hasInitialized.current && !isProcessing.current) {
      fetchProducts();
    }
  }, [mounted, fetchProducts]);
